...
func NewCmdLogin(f cmdutils.Factory) *cobra.Command {
	...
	cmd.Flags().BoolVar(&tokenStdin, "stdin", false, "Read token from standard input. (default false)")
	cmd.Flags().BoolVar(&opts.UseKeyring, "use-keyring", false, "Store token in your operating system's keyring. (default false)")
	cmd.Flags().BoolVar(&opts.WebLogin, "web", false, "Skip the login type prompt and use web/OAuth login. (default false)")
