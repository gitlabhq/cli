...
func genWebDocs(glabCli *cobra.Command, basePath, readmePath string) error {
	...
	// Rewrite the environment variable reference in the hand-maintained
	// configuration page, which the pruning step above leaves alone.
	if err := genEnvVarReference(filepath.Join(basePath, "configuration.md")); err != nil {
		return err
	}
...
// Sentinel comments that delimit the auto-generated environment variable
// reference in docs/source/configuration.md.
const (
	envVarTableStart = "<!-- env-vars-start: generated by 'make gen-docs'; do not edit by hand -->"
	envVarTableEnd   = "<!-- env-vars-end -->"
)

func genEnvVarReference(configPath string) error {
	content, err := os.ReadFile(configPath)
	if err != nil {
		return fmt.Errorf("reading %s: %w", configPath, err)
	}
	updated, err := replaceMarkedSection(string(content), envVarTableStart, envVarTableEnd, envVarTable())
	if err != nil {
		return fmt.Errorf("%s: %w", configPath, err)
	}
	return config.WriteFile(configPath, []byte(updated), 0o644)
}

func envVarTable() string {
	var b strings.Builder
	b.WriteString("\n<!-- markdownlint-disable MD044 MD034 -->\n")
	for _, section := range confighelp.EnvVarsByGroup() {
		fmt.Fprintf(&b, "\n### %s\n\n", section.Group)
		b.WriteString("| Variable | Configuration key | Default | Description |\n")
		b.WriteString("|----------|-------------------|---------|-------------|\n")
		for _, ev := range section.Vars {
			configKey := "-"
			if ev.ConfigKey != "" {
				configKey = "`" + ev.ConfigKey + "`"
			}
			defaultValue := "-"
			if ev.Default != "" {
				defaultValue = "`" + ev.Default + "`"
			}
			description := urlwrapper.MDWrap(escapeTableCell(ev.Description))
			fmt.Fprintf(&b, "| `%s` | %s | %s | %s |\n", ev.Name, configKey, defaultValue, description)
		}
	}
...
func replaceMarkedSection(content, startMarker, endMarker, body string) (string, error) {
	...
	if start == -1 || end == -1 || end < start {
		return "", fmt.Errorf("missing or malformed sentinel markers %q / %q", startMarker, endMarker)
	}
	return content[:start+len(startMarker)] + body + content[end:], nil
}
...
func updateReadme(content []byte, cmd *cobra.Command) ([]byte, error) {
	updated, err := replaceMarkedSection(string(content), readmeCommandListStart, readmeCommandListEnd, "\n"+readmeCommandList(cmd))
	if err != nil {
...
func GenRootMarkdownCustom(cmd *cobra.Command, w io.Writer) error {
	...
	buf.WriteString("For every environment variable `glab` reads, see\n")
	buf.WriteString("[environment variables](configuration.md#environment-variables).\n\n")
